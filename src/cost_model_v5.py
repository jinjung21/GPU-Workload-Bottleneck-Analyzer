...
def estimate_feature_cost_v5(
    ...
    dram = _pct(profile_row, "ncu_sol_dram_pct", fallback="ncu_memory_util_pct")
    memory = _pct(profile_row, "ncu_memory_util_pct", fallback="ncu_sol_dram_pct")
    ...
    memory_bound_signal = max(dram, memory) * (1.0 - min(0.95, sm))
    cache_reuse_signal = max(l1, l2) * min(1.0, arithmetic_intensity / max(hardware.ridge_point, 1e-12))
    compute_signal = sm * min(1.0, arithmetic_intensity / max(hardware.ridge_point, 1e-12))
    occupancy_risk = max(0.0, 0.45 - occupancy)
    ...
    ncu_risk_score = min(
        ...
        + 0.20 * occupancy_risk
        + 0.20 * max(0.0, sm - max(dram, memory)),
    )
    predicted_candidate = (
    ...
    return {
        ...
        "risk": _risk_summary(base["risk"], memory_bound_signal, cache_reuse_signal, compute_signal, occupancy_risk),
        "feature_summary": (
            f"{base['feature_summary']}, "
            f"ncu_dram={dram:.2f}, ncu_sm={sm:.2f}, ncu_l2={l2:.2f}, "
            f"ncu_occ={occupancy:.2f}, ncu_risk={ncu_risk_score:.2f}"
        ),
    }
...
def _pct(row: pd.Series, column: str, fallback: str | None = None) -> float:
